...
function statsMiniTableHTML(s){
  const qty = s.quantite_flyers||0, qtyCh = s.quantite_flyers_chateau||0;
  return `<div class="stop-stats-mini">
    <span>Nougaterie : <strong>${qty}</strong>${qty===0?flyReasonTagG(s.raison_zero_nougaterie):''}</span>
    <span>Château : <strong>${qtyCh}</strong>${qtyCh===0?flyReasonTagG(s.raison_zero_chateau):''}</span>
    <span>Nougat dégust. : <strong>${s.nb_nougat_offert||0}</strong></span>
    <span>Entrées : <strong>${s.entree_gratuite||0}</strong></span>
  </div>`;
}
function rappelBadgeHTML(s){
  const rappel = ST.rappels && ST.rappels[s.lieu_id];
  if(!rappel) return '';
  const isRdv = rappel.type==='rdv_commercial';
  const color = isRdv ? '#7B1FA2' : '#E53935';
  const label = isRdv ? '📅 RDV commercial prévu' : '🔁 Repassage prévu';
  return `<div class="stop-addr" style="color:${color};font-weight:600">${label}</div>`;
}
    ...
    :stops.map((s,idx)=>{
      ...
      <div class="stop-list-item" draggable="true" ondragstart="stopDragStart(event,${idx})" ondragover="stopDragOver(event,${idx})" ondrop="stopDrop(event,${idx})" ondragend="stopDragEnd(event)"${(isLivraisonOnly||hasCommande)?' style="background:#FBF3E7"':''}>
        ...
        <div class="stop-info" style="cursor:pointer" onclick="openStopDetail(${s.id})" title="Voir ce qui a été réalisé"><div class="stop-nom">${esc(s.lieu?.nom||'?')}</div>
          ...
          ${(!isLivraisonOnly && !repere && s.statut==='distribue')?statsMiniTableHTML(s):''}
          ${!repere?rappelBadgeHTML(s):''}
...
async function genererSyntheseTournee(){
  ...
  const cols = [
    { key:'num',    label:'#',                       w:7  },
    { key:'photo',  label:'',                         w:14 },
    { key:'lieu',   label:'Lieu',                     w:52 },
    { key:'statut', label:'Statut',                   w:24 },
    { key:'ngt',    label:'Nougaterie',                w:18 },
    { key:'cha',    label:'Château',                   w:18 },
    { key:'deg',    label:'Nougat dég.',                w:16 },
    { key:'ent',    label:'Entrées',                    w:14 },
    { key:'com',    label:'Commentaire / Repassage',    w:0  }
  ];
  const fixedW = cols.filter(c=>c.key!=='com').reduce((s,c)=>s+c.w,0);
  cols.find(c=>c.key==='com').w = (W - M*2) - fixedW;
  let cx = M;
  cols.forEach(c => { c.x = cx; cx += c.w; });
  const numC=cols[0], photoC=cols[1], lieuC=cols[2], statutC=cols[3], ngtC=cols[4], chaC=cols[5], degC=cols[6], entC=cols[7], comC=cols[8];
  ...
  function drawTableHeader(y){
    ...
    doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(90,75,45);
    cols.forEach(c => { if(c.label) doc.text(c.label, c.x+1.5, y+4.2); });
    doc.setDrawColor(200,190,170);
    doc.line(M, y+6, W-M, y+6);
    return y+6;
  }

  const statutColors = {distribue:[39,174,96], ferme:[120,120,120], refus:[229,57,53], absent:[232,135,10], pas_fait:[41,121,255], a_faire:[160,160,160]};
  const statutTextsDist = {a_faire:'Non visité', distribue:'Fait', ferme:'Fermé', refus:'Refus', absent:'Absent', pas_fait:'Pas fait'};
  const statutTextsLiv  = {a_faire:'À livrer', distribue:'Livré', absent:'Non livré', ferme:'Non livré', refus:'Non livré'};
  const flyTagSyn = r => r==='stock_suffisant' ? ' (S)' : r==='refuse' ? ' (R)' : '';

  function buildComLines(s, isLivraisonOnly, cmd, rappelActif, isRdvCommercial, comW){
    const lines = [];
    if(isLivraisonOnly) lines.push(...doc.splitTextToSize('Livraison : '+truncSynthese(cmd?.description||'(non précisé)', 60), comW));
    if(s.commentaire_distributeur) lines.push(...doc.splitTextToSize(truncSynthese(s.commentaire_distributeur, 90), comW));
    if(rappelActif) lines.push(isRdvCommercial ? 'RDV commercial prévu' : 'Repassage prévu');
    if(!lines.length) lines.push('-');
    return lines;
  }

  function measureRowHeight(s){
    const isLivraisonOnly = s.type_arret === 'livraison';
    const cmd = s.commandes_livraison && s.commandes_livraison[0];
    const rappelActif = ST.rappels && ST.rappels[s.lieu_id];
    const isRdvCommercial = rappelActif && rappelActif.type === 'rdv_commercial';
    const lines = buildComLines(s, isLivraisonOnly, cmd, rappelActif, isRdvCommercial, comC.w-3);
    const textBottom = 5 + lines.length*3.6 + 1.5;
    return Math.max(PHOTO+3, 12, textBottom);
  }

  function drawRow(s, y, rowH, idx){
    const isLivraisonOnly = s.type_arret === 'livraison';
    const cmd = s.commandes_livraison && s.commandes_livraison[0];
    const rappelActif = ST.rappels && ST.rappels[s.lieu_id];
    const isRdvCommercial = rappelActif && rappelActif.type === 'rdv_commercial';

    if(idx % 2 === 1){ doc.setFillColor(250,248,244); doc.rect(M, y, W-M*2, rowH, 'F'); }

    doc.setFont('helvetica','bold'); doc.setFontSize(8.5); doc.setTextColor(80,80,80);
    doc.text(String(s.ordre_passage||''), numC.x+2, y+6);

    const img = photoMap[s.id];
    if(img){ try{ doc.addImage(img,'JPEG', photoC.x+1, y+1.5, PHOTO, PHOTO); } catch(e){} }
    else { doc.setDrawColor(225,225,225); doc.setFillColor(248,248,248); doc.rect(photoC.x+1, y+1.5, PHOTO, PHOTO, 'FD'); }

    doc.setFont('helvetica','bold'); doc.setFontSize(8.5); doc.setTextColor(0,0,0);
    doc.text(truncSynthese(s.lieu?.nom||'', 28), lieuC.x+1.5, y+5);
    doc.setFont('helvetica','normal'); doc.setFontSize(7.5); doc.setTextColor(120,120,120);
    doc.text(truncSynthese(s.lieu?.ville||'', 24), lieuC.x+1.5, y+9.3);

    const labels = isLivraisonOnly ? statutTextsLiv : statutTextsDist;
    const col1 = statutColors[s.statut] || [100,100,100];
    doc.setFont('helvetica','bold'); doc.setFontSize(8); doc.setTextColor(col1[0],col1[1],col1[2]);
    doc.text(labels[s.statut]||s.statut, statutC.x+1.5, y+5);
    if(s.heure_validation){
      doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(140,140,140);
      doc.text(new Date(s.heure_validation).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}), statutC.x+1.5, y+9.3);
    }

    const showQty = !isLivraisonOnly && s.statut === 'distribue';
    if(showQty){
      doc.setFont('helvetica','normal'); doc.setFontSize(8.5); doc.setTextColor(40,40,40);
      doc.text((s.quantite_flyers||0)+((s.quantite_flyers||0)===0?flyTagSyn(s.raison_zero_nougaterie):''), ngtC.x+2, y+7);
      doc.text((s.quantite_flyers_chateau||0)+((s.quantite_flyers_chateau||0)===0?flyTagSyn(s.raison_zero_chateau):''), chaC.x+2, y+7);
      doc.text(String(s.nb_nougat_offert||0), degC.x+2, y+7);
      doc.text(String(s.entree_gratuite||0), entC.x+2, y+7);
    } else {
      doc.setFont('helvetica','normal'); doc.setFontSize(8.5); doc.setTextColor(195,195,195);
      [ngtC,chaC,degC,entC].forEach(c => doc.text('-', c.x+2, y+7));
    }
    ...
    let ly = y+5;
    lines.forEach(line => {
      if(line === 'RDV commercial prévu'){ doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(123,31,162); }
      else if(line === 'Repassage prévu'){ doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(229,57,53); }
      else { doc.setFont('helvetica','italic'); doc.setFontSize(7.5); doc.setTextColor(90,90,90); }
      doc.text(line, comC.x+1.5, ly, {maxWidth: comC.w-3});
      ly += 3.6;
    });
  }
  ...
  stops.forEach((s) => {
    const rowH = measureRowHeight(s);
    if(y + rowH > H - 12){
      doc.addPage(); drawHeader(); y = 26; y = drawTableHeader(y); rowIdx = 0;
    }
    drawRow(s, y, rowH, rowIdx);
  ...
  doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(140,140,140);
  doc.text('(S) stock encore suffisant  ·  (R) refusé par l\'établissement', M, H-6);
